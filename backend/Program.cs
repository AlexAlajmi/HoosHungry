...
app.MapPost(
    "/api/sellers/{sellerId}/availability",
    async (
        string sellerId,
    ...
    async (
        ...
        await ExecuteAsync(async () =>
        {
            ...
            var relatedOffers = state
                ...
                .ToList();
            if (
                relatedOffers.Any(item =>
                    item.Id != offer.Id && item.Status == OfferStatus.Accepted
                )
            )
            {
                throw new InvalidOperationException(
            ...
            foreach (
                var sibling in relatedOffers.Where(item =>
            ...
            await InsertNotificationAsync(
                ...
                new NotificationItem
                {
                    ...
                    Message =
                        $"{offer.SellerName} accepted your {offer.Price:C} meal exchange request.",
                    CreatedAtUtc = DateTime.UtcNow,
                },
                cancellationToken
            );
...
static async Task<IResult> ExecuteAsync(Func<Task<object>> action)
{
    try
    {
...
static async Task<MarketplaceDashboardState> LoadDashboardStateAsync(
    HttpClient client,
    SupabaseSettings settings,
    CancellationToken cancellationToken
)
    ...
    return new MarketplaceDashboardState
    {
        ...
        Metrics = new DemoMetrics
        {
            ...
            PlatformEscrowTotal = orders
                .Where(order =>
                    !order.FundsReleasedToSeller && order.Status != OrderStatus.Declined
                )
...
static Task PatchOfferStatusAsync(
    HttpClient client,
    SupabaseSettings settings,
    OfferRecord offer,
    CancellationToken cancellationToken
...
static Task InsertTrackingEventAsync(
    HttpClient client,
    SupabaseSettings settings,
    string orderId,
...
static Task InsertNotificationAsync(
    HttpClient client,
    SupabaseSettings settings,
...
static Task InsertWithdrawalAsync(
    HttpClient client,
    SupabaseSettings settings,
...
static async Task<List<T>> GetRowsAsync<T>(
    HttpClient client,
    SupabaseSettings settings,
    string table,
...
static async Task<List<T>> InsertRowsAsync<T>(
    HttpClient client,
    SupabaseSettings settings,
    string table,
    object payload,
...
static async Task<List<T>> PatchRowsAsync<T>(
    HttpClient client,
    SupabaseSettings settings,
    string table,
    IReadOnlyDictionary<string, string> query,
...
static void EnsureSupabaseConfigured(SupabaseSettings settings)
{
    ...
    {
        throw new InvalidOperationException(
            "Supabase URL is missing or invalid. Set Supabase:Url."
        );
    }
...
static UserAccount ToUser(SupabaseUserRow row)
{
...
static OfferRecord ToOffer(SupabaseOfferRow row, IReadOnlyDictionary<string, UserAccount> users)
{
...
static (string OrderId, TrackingEvent Event) ToTrackingPair(SupabaseTrackingEventRow row)
{
    return (
        row.OrderId,
        new TrackingEvent
...
static NotificationItem ToNotification(SupabaseNotificationRow row)
{
...
static WithdrawalRecord ToWithdrawal(
...
static string BuildTrackingLabel(OrderStatus status)
{
...
static string NewId(string prefix) => $"{prefix}-{Guid.NewGuid():N}";
